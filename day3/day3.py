...
df = df.map(lambda x: pd.to_numeric(x, errors='coerce') if isinstance(x, (int, float)) else -99999)
...
sum_positive = df[df > 0].sum().sum()

print("Sum of positive values:", int(sum_positive))
